...
int main(void)
{
    static bool USBWelcomeMessageSent = false;
	static int i;
	static uint32 vAve;
	static char buff[64];
  char testBuff[2];
...
bool show_ver = false;
    ...
    if(!USBWelcomeMessageSent){
        
    USBWelcomeMessageSent = putString("\r\nVariable Load: V1.0 Startup\r\n");
    }
		...
		if (systemTimer - 20 > SlowTick)
		{
			...
			if (floatBuff[incCharIndex] == '\0')
			{
				...
				case 'R':
					...
					break;
                case '?':
                    if(show_ver){show_ver = false;}else {show_ver = true;}
